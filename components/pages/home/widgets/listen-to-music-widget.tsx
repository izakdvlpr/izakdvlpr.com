...
import type { ListenToMusic } from "@/lib/api";

interface ListenToMusicWidgetProps {
	listenToMusic: ListenToMusic;
}

export function ListenToMusicWidget({
	listenToMusic,
}: ListenToMusicWidgetProps) {
	const track = listenToMusic.now ?? listenToMusic.last;

	return (
		<div className="md:col-span-2 col-span-1 h-[150px] p-5 flex flex-col gap-2 rounded-md bg-gray-100">
			<div className="flex items-center gap-2">
				<h1 className="text-md font-medium">
					{listenToMusic.now ? "Listening to music" : "Last listened track"}
				</h1>
