...
export function DiscordRecentPlayedWidget({ discord }: DiscordRecentPlayedWidgetProps) {
  return (
    <div className="md:col-span-2 col-span-1 flex flex-col gap-2 p-5 rounded-md bg-gray-100">
      ...
      {discord?.recentPlayed && discord.recentPlayed.length > 0 ? (
          ...
          {discord.recentPlayed.map((game, index) => (
            <li key={index} className="flex items-center gap-3">
              ...
              <div className='max-w-75 flex flex-col truncate'>
                <p className="text-sm font-medium text-ellipsis overflow-hidden">{game.gameName}</p>
        ...
        <p className="mt-4 text-sm text-gray-500">No recent played games available.</p>
      )}
    </div>
  )
}