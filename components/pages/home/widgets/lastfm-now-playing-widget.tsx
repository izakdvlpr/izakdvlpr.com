	...
	return (
		<div className="md:col-span-2 col-span-1 h-37.5 p-5 flex flex-col gap-2 rounded-md bg-gray-100">
			<div className="flex items-center gap-2">
				<h1 className="text-md font-medium">{lastfm.nowPlaying ? "Listening to music" : "Last listened track"}</h1>
			...
			{track ? (
				<div className="flex gap-4">
					...
					<div className="max-w-75 flex flex-col truncate">
						...
						<p title={track.album} className="text-sm text-ellipsis overflow-hidden">
							{" "}
							{track.album}
						</p>

						<p title={track.artist} className="text-sm text-ellipsis overflow-hidden">
							{track.artist}
						</p>
					</div>
				</div>
